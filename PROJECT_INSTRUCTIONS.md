...
3. web_fetch — только для внешних ссылок из INDEX.md. «Читай молча» и блок «Использованные скиллы» — в силе.
...
Если **папки** `people/<active-user>/` не существует — это ошибка настройки (username в инструкциях не совпадает с именем папки в `people/`). Скажи об этом **первой строкой ответа**, попроси исправить настройку и продолжай в нейтральном тоне — не маскируй проблему мягким напоминанием. Если папка есть, но отдельный файл отсутствует или пуст — продолжай работу и в конце ответа упомяни, что файл стоит заполнить.
...
- **Отсутствующий или пустой файл — не стоп.** Если любой файл из шагов 1A–6 отсутствует или пуст — продолжай без него на общих принципах и явно назови пробел в блоке «Использованные скиллы» строкой `не найдено: <путь>`.
