 ...
 *   разобрать каждое поле: fills/strokes, радиусы, отступы/gap, strokeWeight, width/height (если привязаны),
 *   у TEXT — текстовый стиль и типографика, эффекты — и классифицировать:
 ...
 *     old              — привязано, но не к 🧩 Tokens (локальные коллекции Core kit, [OLD] Mўlo DS и т.п.)
 ...
 *   имена переменных совпадают с Tokens (bg/brand/primary и т.п.). Текст/эффект-стили — по key стиля (список ниже,
 *   выгружен из файла 🧩 Tokens 2026-09-26; при добавлении стилей в Tokens — обновить TOKENS_TEXT_STYLES/EFFECT_STYLES).
 *
 ...
 *   variables.getVariableByIdAsync / getVariableCollectionByIdAsync, teamLibrary.getAvailableLibraryVariableCollectionsAsync /
 *   getVariablesInLibraryCollectionAsync. Никаких setter'ов, setBoundVariable, create/remove/rename, setPluginData, import*.
 *
 ...
 *   MODE       — 'summary': счётчики + список старых переменных + raw-значения + инстансы + сверка с components-export (compat);
 *                'findings': дедуплицированные находки, страница [OFFSET, OFFSET+LIMIT).
 *   OFFSET, LIMIT — пейджинг находок. Ответ use_figma режется ~20 KB → LIMIT 40–60. В ответе есть total и next
 *                (next = null — последняя страница).
 *   INCLUDE_NA — включать ли n/a в findings (в summary они считаются всегда).
 *   CLASSES    — фильтр классов для findings, напр. ['old','raw']; [] — все.
 *
 * Как запускать: Figma MCP → use_figma(fileKey="PzDKr0egYAQaq9oI0dbJJP", code=<этот файл с подставленными параметрами>).
 *   Сначала MODE='summary', потом MODE='findings' с OFFSET=0, LIMIT, … пока next != null.
 *
 * Формат находки: {variant, layerPath, nodeId, field, class, current, hex?, value?, via?, collection?, twin?, count, variants, nVariants}
 *   layerPath — путь от варианта (сам вариант = «.»), дедуп по (layerPath, field, class, current) между вариантами;
 *   variant/nodeId — первое вхождение; variants — до 3 имён + «… +N»; count — число совпавших полей; nVariants — вариантов.
 *   current: имя переменной/стиля или сырое значение; hex — итоговый цвет (для привязанных — по valuesByMode дефолтного
 *   мода коллекции с разворотом алиасов; если недоступно — текущий цвет краски), альфа — « @NN%»; «*» в конце hex —
 *   у одинаковой находки в разных вариантах разный цвет. value — итоговое число/строка у числовых полей.
 ...
 *   библиотеки; twin — коллекция 🧩 Tokens, где есть переменная с тем же именем (кандидат на перепривязку), или null.
 ...
 *   width/height/min/max/opacity — только если привязаны; TEXT: textStyle, fontSize/lineHeight/letterSpacing/fontFamily
 *   и вес (поле fontWeight, или fontStyle — если вес привязан через fontStyle); при наличии текстового стиля
 *   непривязанные поля типографики не считаются — их задаёт стиль; effects: effectStyle или каждый видимый эффект.
 *   Сам INSTANCE аудируется как нода (его fills/strokes/радиусы) + попадает в summary.instances с именем main component.
 ...
 *   summary.compat считает T/O/U ровно по тем правилам.
 */
...
const CLASSES = [];                // [] — все; напр. ['old','raw']
...
let twinIdx = null; // имя переменной → коллекция Tokens (грузится лениво, только если есть old)
async function twinOf(name) {
  if (!twinIdx) {
    twinIdx = {};
    for (const key of Object.keys(TOK)) {
      try { for (const lv of await figma.teamLibrary.getVariablesInLibraryCollectionAsync(key)) if (!twinIdx[lv.name]) twinIdx[lv.name] = TOK[key]; } catch (e) {}
    }
  }
  return twinIdx[name] || null;
}
...
async function clsVar(id) {
  ...
  let r = { class: 'old', current: '?' + id, collection: 'missing-var' };
  try {
    ...
    if (v) {
      ...
      else { r.class = 'old'; r.collection = col ? col.name + (col.remote ? ' [' + (LIBNAME[col.key] || 'remote, библиотека не подключена') + ']' : ' (local)') : 'no-collection'; r.twin = await twinOf(v.name); }
    }
...
async function styleInfo(id, kind) {
  ...
  let r = { class: 'old', current: '?style ' + id };
  try {
    const s = await figma.getStyleByIdAsync(id);
    if (s) {
      const tok = s.remote && (kind === 'text' ? TXT_STYLE_SET : EFF_STYLE_SET).has(s.key);
      r = { class: tok ? 'tokens-semantic' : 'old', current: s.name + (s.remote ? '' : ' (local)') };
    }
  } catch (e) {}
  return (sCache[id] = r);
}
...
// ---------- сбор полей ----------
const recs = [];         // {variant, path, nodeId, field, class, current, hex, collection, twin}
...
async function paintField(ctx, field, p) {
  ...
  if (p.type === 'SOLID') {
    ...
    if (a && a.id) {
      ...
      const hx = c.value && typeof c.value === 'object' && 'r' in c.value ? hex(c.value) : hex(p.color, p.opacity);
      return push(ctx, field, { class: c.class, current: c.current, hex: hx, via: c.via, collection: c.collection, twin: c.twin });
    }
  ...
  if (p.type && p.type.indexOf('GRADIENT') === 0) {
    ...
    if (b) { const c = await clsVar(b.id); return push(ctx, field, { class: c.class, current: p.type + ' ' + c.current + (stops.length > 1 ? ' (+' + (stops.length - 1) + ' stops)' : ''), hex: hx, collection: c.collection, twin: c.twin }); }
...
async function numField(ctx, field, alias, value, opts) {
  ...
  if (a && a.id) {
    const c = await clsVar(a.id);
    return push(ctx, field, { class: c.class, current: c.current, value: r2(c.value), via: c.via, collection: c.collection, twin: c.twin });
  }
  if (opts && opts.onlyBound) return;
  ...
  return push(ctx, field, { class: 'raw', current: typeof value === 'object' ? fmtLH(value) : r2(value) });
}
...
const DIM = ['width', 'height', 'minWidth', 'maxWidth', 'minHeight', 'maxHeight', 'opacity'];
...
async function auditNode(x, ctx) {
  ...
  // размеры/opacity — только привязанные
  for (const k of DIM) if (bv[k]) await numField(ctx, k, bv[k], x[k], { onlyBound: true });
  // типографика
  if (x.type === 'TEXT') {
    let styleIds = [];
    ...
    for (const k of TXT) {
      let val, field = k, alias = bv[k];
      if (k === 'fontFamily') val = isMixed(x.fontName) ? x.fontName : x.fontName.family;
      ...
      await numField(ctx, field, alias, val, { onlyBound: hasStyle });
    }
  }
  // эффекты
  if ('effects' in x) {
    ...
    if (esid && !isMixed(esid)) { const s = await styleInfo(esid, 'effect'); push(ctx, 'effectStyle', { class: s.class, current: s.current }); }
      ...
      for (let i = 0; i < x.effects.length; i++) {
        ...
        if (a && a.id) { const c = await clsVar(a.id); push(ctx, 'effect', { class: c.class, current: c.current + ' (' + desc + ')', hex: e.color ? hex(e.color) : undefined, collection: c.collection, twin: c.twin }); }
...
const CIDX = { 'tokens-semantic': 0, 'tokens-primitive': 0, old: 1 };
...
async function walk(x, ctx, isRoot) {
  ...
  for (const ch of x.children) await walk(ch, { variant: ctx.variant, id: ch.id, path: (isRoot ? '' : ctx.path + ' / ') + ch.name }, false);
}
for (const v of variants) await walk(v, { variant: v.name, id: v.id, path: '.' }, true);
...
for (const r of recs) {
  ...
  if (CLASSES.length && !CLASSES.includes(r.class)) continue;
  const k = r.path + '\u0001' + r.field + '\u0001' + r.class + '\u0001' + r.current;
  let d = dmap.get(k);
  if (!d) {
    ...
    if (r.collection && r.class === 'old') d.collection = r.collection;
    if (r.class === 'old') d.twin = r.twin || null;
    d.count = 0; d._v = [];
    dmap.set(k, d);
...
const ORDER = { old: 0, raw: 1, 'tokens-primitive': 2, 'tokens-semantic': 3, 'n/a': 4 };
const findings = [...dmap.values()].sort((a, b) => ORDER[a.class] - ORDER[b.class] || (a.layerPath < b.layerPath ? -1 : a.layerPath > b.layerPath ? 1 : 0) || (a.field < b.field ? -1 : 1));
for (const d of findings) { d.variants = d._v.length > 3 ? d._v.slice(0, 3).concat('… +' + (d._v.length - 3)) : d._v; d.nVariants = d._v.length; delete d._v; }

if (MODE === 'summary') {
  const oldVars = {}, rawVals = {};
  for (const r of recs) {
    if (r.class === 'old') {
      const k = r.current + '|' + r.collection;
      const o = oldVars[k] || (oldVars[k] = { name: r.current, collection: r.collection, twin: r.twin || null, value: r.hex || r.value, via: r.via, uses: 0, fields: [] });
  ...
  return {
    summary: {
      ...
      byClass, byFieldType, findingsTotal: findings.length,
    },
