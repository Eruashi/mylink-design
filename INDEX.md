...
Тестовые запросы для проверки роутинга. Заведён 2026-07-02 вместе с
`skills/product-design/`; покрывает все 8 доменных скиллов (минимум 3
позитивных кейса + 1 негативный на границу с соседним доменом каждый).
Проверка пока только ручная сверка с `SKILL.md`, не реальный прогон через
роутер. Домен `meta` не покрыт.
