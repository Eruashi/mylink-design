...
- `tasks/<slug>.md` — карточка таски, если в запросе названа таска или фича, у которой есть карточка (список — файлы в `tasks/`; например «Табель» → `tasks/tabel.md`). Карточка — актуальная версия ТЗ: при расхождении с ТЗ БА или прошлым контекстом верна карточка. Если пользователь сообщает решение («решили X», «договорились») — в Claude Code обнови карточку (логика + строка в журнале); в claude.ai предложи текст правки.
