...
1. **Контекст.** Найди компонент в `ds/components.md` (node, % Tokens). Прочитай `figma-status/<файл>/<компонент>.md`, если есть.
2. **Сбор.** Запусти `ds/_scripts/component-token-audit.js` через `use_figma` (только чтение): сначала `MODE='summary'`, затем все страницы `findings`. Без Figma MCP (claude.ai) — попроси у пользователя выгрузку или скриншоты и честно снизь покрытие.
3. **Классификация.** Каждое поле: `tokens-semantic` ✅ · `tokens-primitive` ⚠ · `old` ❌ · `raw` ❌.
4. **Подбор замены** для ⚠ и ❌:
   - найди в `ds/tokens.md` семантический токен той же роли с тем же hex (или числом);
   - уверенность: **высокая** — роль и значение совпали; **средняя** — роль совпала, значение отличается ≤ 1 шаг палитры; **низкая** — совпал только hex;
   - ничего не подошло → «Нет в источнике».
5. **Отчёт** по `templates/review-report.md`: одна строка на группу одинаковых находок (слой без имени варианта + поле + текущее значение), с числом вхождений. Решение: «исправить» / «отдельная таска» / «принять».
...
## Формат вывода
`templates/review-report.md`. Колонки таблицы: Где (слой) · Поле · Сейчас · Предлагаю (токен) · Уверенность · Вхождений. Строка покрытия обязательна.
