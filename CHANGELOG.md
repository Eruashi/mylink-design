...
### Added
- `templates/task-card.md` — шаблон карточки таски (живая версия ТЗ + журнал решений).
- `tasks/tabel.md` — карточка Табеля по ТЗ БА.

### Changed
- `PROJECT_INSTRUCTIONS.md` — шаг 1A: названа таска → читать `tasks/<slug>.md`; карточка приоритетнее ТЗ.

## 2026-09-25 — Данные дизайн-системы в `ds/`
...
### Changed
- `PROJECT_INSTRUCTIONS.md` — шаг 4A: вопросы про компоненты/токены/иллюстрации опираются на `ds/`; имена только из реестра, «Не использовать» не предлагать.
