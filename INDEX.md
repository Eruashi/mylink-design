...
> Локальные файлы читаются напрямую (Project Knowledge / подключённая папка), **не** по сети.
> `web_fetch` — только для внешних raw-ссылок из этого файла (Шаг 6 `PROJECT_INSTRUCTIONS.md`).

---

## Навигация

- [Локальные ресурсы](#локальные-ресурсы) — `team/`, `people/`, `skills/`, `mirrors/`, `templates/`, `frameworks/`, `evals/`
...
Тестовые запросы для проверки роутинга. Покрывает все 8 доменных скиллов
(минимум 3 позитивных кейса + 1 негативный каждый), домен `meta` (3 кейса —
по одному на зеркало) и спец-кейсы: мульти-домен, «ничего не триггерится»,
цепочка. Проверка пока только ручная сверка с `PROJECT_INSTRUCTIONS.md`
(Шаг 2) и `SKILL.md`, не реальный прогон через роутер.
