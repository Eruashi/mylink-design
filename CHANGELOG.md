...
### Changed
- `PROJECT_INSTRUCTIONS.md` — раздел «Формат ответа» (вывод → суть → следующий шаг, ~150 слов, строка покрытия в ревью); блок «Использованные скиллы» — только по запросу; пробелы в файлах не упоминаются, если не меняют ответ; `tone-of-voice.md` задаёт стиль текстов от имени пользователя, не формат ответов.
